...
- double GetGSRFeature(string featureName) – returns the value (as double number) of a specific GSR feature (name can be SCRArousalArea, SCRAchievedArousalLevel, SCLAchievedArousalLevel and MovingAverage). It returns -1 if the device is not started (active);
