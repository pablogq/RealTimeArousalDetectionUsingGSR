...
public class CacheSignalData
{
    ...
    private static void FillTestGSRDataFromFile()
    {
        using (StreamReader sr = new StreamReader(ConfigurationManager.AppSettings.Get("TestData"), Encoding.Default))
        {
            string text = sr.ReadToEnd();
            string[] lines = text.Split(';');
            for(int i = 0; i < lines.Length; i++)
            {
                if(!String.IsNullOrEmpty(lines[i])) AddChannelCacheValue(0, Int32.Parse(lines[i]));
            }
        }
    }
    ...
    public static void AddChannelCacheValue(int channel, int cacheValue)
    {
        ...
        //Logger.Log((DateTime.Now - DateTime.MinValue).TotalMilliseconds + ", value: " + cacheValue);
    ...
    public static List<double> GetAllForChannel(int channel)
    {
        ...
        else if (channelsCache != null && channelsCache.ContainsKey(channel))
        {
